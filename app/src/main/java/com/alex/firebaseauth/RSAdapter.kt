...
class RSAdapter(val mCtx : Context, val layoutResId: Int, val rsList : List<RS>): ArrayAdapter<RS> (mCtx, layoutResId,
 rsList) {
    ...
    fun showUpdateDialog(rs: RS){
        ...
        builder.setPositiveButton("Update"){ p0 , p1 ->
            val dbRS = FirebaseDatabase.getInstance().getReference("RS")
            ...
            if(alamat.isEmpty()){
                et_Alamat.error = "Mohon Alamat Diisi"
                et_Alamat.requestFocus()
                return@setPositiveButton
            }
            if(latitude.isEmpty()){
                et_Latitude.error = "Mohon Latitude Diisi"
                et_Latitude.requestFocus()
                return@setPositiveButton
            }
            if(longitude.isEmpty()){
                et_Longitude.error = "Mohon Longitude Diisi"
                et_Longitude.requestFocus()
                return@setPositiveButton
            }
            val rs = RS(rs.id, nama, alamat, latitude, longitude)
            dbRS.child(rs.id!!).setValue(rs)

            Toast.makeText(mCtx,"Data Berhasil Di Update", Toast.LENGTH_SHORT).show()
        }
        builder.setNeutralButton("Delete"){p0,p1 ->
            val dbRS = FirebaseDatabase.getInstance().getReference("RS").child(rs.id.toString())
            dbRS.removeValue()
